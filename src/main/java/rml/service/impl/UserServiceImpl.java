...
public class UserServiceImpl implements UserServiceI {
    ...
    public User login(User user) {
        Tuser t = userDao.get("from Tuser t where t.cname = ? and t.cpwd = ?",
    ...
    public void delete(String ids) {
        if (ids != null) {
            for (String id : ids.split(",")) {
                ...
                if (u != null) {
                    userroleDao.executeHql("delete Tusertrole t where t.tuser = ?",
                            new Object[] { u });
                    userDao.delete(u);
                }
            }
    ...
    public List<User> combobox(User user) {
        ...
        return changeModel(userDao.find("from Tuser t where t.cname like ?", new Object[] { "%%"
                + q.trim() + "%%" }, 1, 10));
    }
    ...
    public List<RoleChart> countRole() {
        ...
        String hql = "select r.cname as name,count(u.cid) as count from Trole r left join Tusertrole u on r.cid=u.croleid group by r.cname";
