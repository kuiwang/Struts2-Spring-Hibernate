...
public class OnlineListener implements ServletContextListener, ServletContextAttributeListener,
    ...
    /**
     * called when add attributes to session (called after successful
     * login)
     */
    public void attributeAdded(HttpSessionBindingEvent evt) {
        ...
        if (ResourceUtil.getSessionInfoName().equals(name)) {// the name of attribute saved in session is 'sessionInfo'
            ...
            if (sessionInfo != null) {
                ...
                onlineService.saveOrUpdateTonlineByLoginNameAndIp(sessionInfo.getLoginName(),
                        sessionInfo.getIp());
            }
        }
    }
    ...
    public void sessionDestroyed(HttpSessionEvent evt) {
        ...
        if (session != null) {
            ...
            if (sessionInfo != null) {
                OnlineServiceI onlineService = (OnlineServiceI) ctx.getBean("onlineService");
                onlineService.deleteTonlineByLoginNameAndIp(sessionInfo.getLoginName(),
                        sessionInfo.getIp());
            }
        }
    }
